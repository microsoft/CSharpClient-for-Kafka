...
namespace Kafka.Client.Producers.Partitioning
{
    ...
    internal class ZKBrokerPartitionInfo : IBrokerPartitionInfo, IZooKeeperStateListener
    {
        ...
        private void InitializeBrokers()
        {
            ...
            IEnumerable<string> brokerIds = this.zkclient.GetChildrenParentMayNotExist(ZooKeeperClient.DefaultBrokerIdsPath);
            foreach (var brokerId in brokerIds)
            {
                ...
                this.brokers.Add(id, new Broker(id, parts[0], port));
            }
        }
        ...
        private void InitializeTopicBrokerPartitions()
        {
            ...
            IEnumerable<string> topics = this.zkclient.GetChildrenParentMayNotExist(ZooKeeperClient.DefaultBrokerTopicsPath);
            foreach (string topic in topics)
            {
                string brokerTopicPath = ZooKeeperClient.DefaultBrokerTopicsPath + "/" + topic;
                IEnumerable<string> brokersPerTopic = this.zkclient.GetChildrenParentMayNotExist(brokerTopicPath);
        ...
        public void HandleStateChanged(ZooKeeperStateChangedEventArgs args)
        {
            ...
            Guard.Assert<ArgumentException>(() => args.State != ZkClientState.Unknown);
